# -*- coding: utf-8 -*-

from . import res_company
from . import res_partner
from . import product
from . import sale
from . import purchase
from . import stock
from . import stock_picking
from . import mrp
from . import mrp_production
from . import is_fiche_travail
from . import bsa_fnc
from . import is_export_compta
from . import is_accident_travail
from . import is_personnel_present
from . import is_fiche_controle
from . import is_balance_agee
from . import is_contrat_fournisseur
from . import is_liste_manquants
from . import is_paye
from . import is_sale_order_line
from . import is_picking_line
from . import is_mrp_bom_line
from . import is_derniere_commande_achat
from . import account_move_line
from . import account_move
from . import is_import_nomenclature
from . import bsa_stock_a_date
from . import email_template
from . import ir_mail_server
from . import hr
from . import project
from . import is_bsa_tracabilite
from . import is_badge
from . import is_pointage
from . import is_account_move_line
from . import is_gamme_generique
from . import is_devis_parametrable
from . import is_outillage
from . import is_systeme_gestion
from . import is_simulation_livrable



